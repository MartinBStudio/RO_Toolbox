...
public class BuffsManagerService implements GameResourceService, ICommonMethods {
  ...
  private void normalizeInstalledManifest(Path destination) throws IOException {
    ...
    Path preferredManifest =
        Files.exists(currentManifest) && Files.isRegularFile(currentManifest)
            ? currentManifest
            : null;
    if (preferredManifest == null
        && Files.exists(legacyManifest)
        && Files.isRegularFile(legacyManifest)) {
      preferredManifest = legacyManifest;
    }
    if (preferredManifest == null
        && Files.exists(legacyBuffsManifest)
        && Files.isRegularFile(legacyBuffsManifest)) {
    ...
    if (Files.exists(legacyManifest)
        && Files.isRegularFile(legacyManifest)
        && !legacyManifest.equals(currentManifest)) {
      Files.deleteIfExists(legacyManifest);
    }
    if (Files.exists(legacyBuffsManifest)
        && Files.isRegularFile(legacyBuffsManifest)
        && !legacyBuffsManifest.equals(currentManifest)) {
