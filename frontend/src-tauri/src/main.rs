...
fn get_backend_url(state: tauri::State<BackendState>) -> Result<String, String> {
    ...
    let port_text = std::fs::read_to_string(&port_file)
        .map_err(|e| {
            format!(
                "Failed to read backend port file '{}': {e}",
                port_file.display()
            )
        })?;
...
fn main() {
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            match spawn_backend_with_retry(
                app.handle(),
                &jar_path,
                5,
                ...
                Ok((child, port)) => {
                    ...
                    *state
                        .port
                        .lock()
                        .expect("backend lock poisoned") = Some(port);
                }

                Err(e) => {
                    eprintln!(
                        "[RO Toolbox] Failed to start backend: {e}"
                    );
                }
            }
...
fn stop_backend_child(state: &BackendState) {
    ...
    *state
        .port
        .lock()
        .expect("backend lock poisoned") = None;
...
fn terminate_process_tree_windows(pid: u32) {
    ...
    if let Err(err) = command.status() {
        eprintln!(
            "[RO Toolbox] Failed to terminate backend process tree \
             (PID {}): {}",
            pid, err
        );
    }
            ...
            Ok(mut child) => {
                let stdout = match child.stdout.take() {
                    ...
                    None => {
                        ...
                        last_err =
                            "Failed to capture backend stdout".to_string();
                ...
                let stderr = match child.stderr.take() {
                    ...
                    None => {
                        ...
                        last_err =
                            "Failed to capture backend stderr".to_string();
                ...
                thread::spawn(move || {
                    ...
                    for line in reader.lines().flatten() {
                        println!("[RO Toolbox Backend] {line}");

                        if let Some(port) = extract_tomcat_port(&line) {
                ...
                thread::spawn(move || {
                    ...
                    for line in reader.lines().flatten() {
                        eprintln!("[RO Toolbox Backend] {line}");
                    }
                });

                match port_receiver.recv_timeout(Duration::from_secs(30)) {
                    Ok(port) => {
                        println!(
                            "[RO Toolbox] Backend successfully started \
                             on port {port}"
                        );
                    ...
                    Err(_) => {
                        ...
                        last_err = format!(
                            "Backend did not report its Tomcat port \
                             (attempt {}/{})",
                            attempt + 1,
                            retries + 1
                        );
...
fn extract_tomcat_port(line: &str) -> Option<u16> {
    let marker = "Tomcat started on port ";
...
fn find_backend_jar(app_handle: &AppHandle) -> Result<PathBuf, String> {
    ...
    Err(
        "RO_Toolbox.jar not found. Build backend jar and copy it to \
         frontend/src-tauri/resources first."
            .to_string(),
    )
}
...
fn sync_taskbar_quick_launch_windows(
    ...
    let port = state
        ...
        .expect("backend lock poisoned")
        .ok_or_else(|| {
            "Backend port is not available".to_string()
        })?;
    ...
    for account in accounts {
        ...
        tasks.push((
            name.to_string(),
            format!(
                "http://localhost:{}/api/login/{}/launch",
                port,
                account.id
            ),
...
fn update_windows_jump_list(
    ...
    unsafe {
        CoInitializeEx(None, COINIT_APARTMENTTHREADED)
            ...
            .map_err(|err| {
                format!(
                    "Failed to initialize COM for taskbar menu: {err}"
                )
            })?;
    }
...
fn update_windows_jump_list_inner(
    ...
    let destination_list: ICustomDestinationList = unsafe {
        ...
        .map_err(|err| {
            format!(
                "Failed to create Windows jump list: {err}"
            )
        })?
    };
    ...
    let _removed_destinations: IObjectArray = unsafe {
        destination_list
            ...
            .map_err(|err| {
                format!(
                    "Failed to initialize taskbar menu: {err}"
                )
            })?
    };
    ...
    if tasks.is_empty() {
        unsafe {
            destination_list
                ...
                .map_err(|err| {
                    format!(
                        "Failed to clear taskbar quick launch \
                         group: {err}"
                    )
                })?;
        }
    ...
    let quick_launch_collection: IObjectCollection = unsafe {
        ...
        .map_err(|err| {
            format!(
                "Failed to create taskbar menu collection: {err}"
            )
        })?
    };

    let wscript_path =
        HSTRING::from(r"C:\Windows\System32\wscript.exe");

    let launcher_script =
        ensure_taskbar_launcher_script()?;

    let launcher_script_path =
        launcher_script.to_string_lossy().to_string();

    let app_exe = std::env::current_exe()
        .map_err(|err| {
            format!(
                "Failed to resolve app executable path: {err}"
            )
        })?;

    let max_entries = max_slots.max(1) as usize;

    for (title, endpoint, icon_path) in
        tasks.into_iter().take(max_entries)
    {
        let task_item: IShellLinkW = unsafe {
            ...
            .map_err(|err| {
                format!(
                    "Failed to create taskbar menu item: {err}"
                )
            })?
        };
        ...
        unsafe {
            task_item
                ...
                .map_err(|err| {
                    format!(
                        "Failed to set taskbar command path: {err}"
                    )
                })?;
            ...
            task_item
                ...
                .map_err(|err| {
                    format!(
                        "Failed to set taskbar command arguments: {err}"
                    )
                })?;
            ...
            task_item
                ...
                .map_err(|err| {
                    format!(
                        "Failed to set taskbar item description: {err}"
                    )
                })?;
            ...
            task_item
                ...
                .map_err(|err| {
                    format!(
                        "Failed to set taskbar item icon: {err}"
                    )
                })?;
        }
        ...
        unsafe {
            quick_launch_collection
                ...
                .map_err(|err| {
                    format!(
                        "Failed to add taskbar menu item: {err}"
                    )
                })?;
        }
    }

    let object_array: IObjectArray =
        quick_launch_collection
            .cast()
            .map_err(|err| {
                format!(
                    "Failed to finalize taskbar items: {err}"
                )
            })?;
    ...
    unsafe {
        destination_list
            .AppendCategory(
                &HSTRING::from("Quick launch"),
                &object_array,
            )
            .map_err(|err| {
                format!(
                    "Failed to update taskbar quick launch \
                     group: {err}"
                )
            })?;
        ...
        destination_list
            ...
            .map_err(|err| {
                format!(
                    "Failed to commit taskbar menu: {err}"
                )
            })?;
    }
...
fn set_shell_link_title(
    ...
    let property_store: IPropertyStore = shell_link
        ...
        .map_err(|err| {
            format!(
                "Failed to open taskbar item properties: {err}"
            )
        })?;
    ...
    unsafe {
        property_store
            ...
            .map_err(|err| {
                format!(
                    "Failed to set taskbar item title: {err}"
                )
            })?;
        ...
        property_store
            ...
            .map_err(|err| {
                format!(
                    "Failed to save taskbar item title: {err}"
                )
            })?;
    }
...
fn ensure_taskbar_launcher_script() -> Result<PathBuf, String> {
    let script_path =
        std::env::temp_dir()
            .join("ro_toolbox_taskbar_launch.vbs");
    ...
    fs::write(&script_path, script)
        .map_err(|err| {
            format!(
                "Failed to prepare taskbar launcher script: {err}"
            )
        })?;
...
mod tests {
    use super::{
        extract_tomcat_port,
        taskbar_icon_file_name,
    };

    #[test]
    fn extracts_tomcat_port() {
        assert_eq!(
            extract_tomcat_port(
                "Tomcat started on port 61266 (http) with context path '/'"
            ),
            Some(61266)
        );
    }
    ...
    #[test]
    fn returns_none_for_non_tomcat_line() {
        assert_eq!(
            extract_tomcat_port(
                "Started RO_ToolboxApplication"
            ),
            None
        );
    }
    ...
    fn falls_back_to_default_taskbar_icon_file() {
        assert_eq!(
            taskbar_icon_file_name("unknown"),
            "person.ico"
        );
        assert_eq!(
            taskbar_icon_file_name(""),
